...
public class SchoolList extends RecyclerView.Adapter<SchoolList.MyViewHolder> {

    private List<School> list;
    private Activity activity;

    public SchoolList(List<School> list, Activity activity) {
        this.list = list;
    ...
    @Override public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        ...
        holder.gender.setText(list.get(position).getLocation());
        holder.contact.setText(list.get(position).getTiming());
        Glide.with(activity).load(ApiClient.BASE_URL_SCHOOL_IMAGES+list.get(position).getImage()).into(holder.profile_image);
